...
- [x] The rating control is operable by keyboard alone and announces its value; asserted in the
      accessibility e2e pass. Unit-tested in full (`packages/styles/test/star-rating.test.tsx`) and
      now e2e-tested against a real seeded product page (`accessibility.spec.ts`'s S4 test, unblocked
      by TASK-0006's seed data being wired into the e2e harness) — arrow-key and Home/End navigation,
      and the `aria-live` announced value, verified end to end against a real browser.
- [x] The aggregate display states when it was last computed rather than implying it is live.
- [x] A Playwright spec covers: browse, open a product, submit a review, see it in the list.
      `catalogue-and-review.spec.ts`, unblocked by TASK-0006's seed data — targets the seeded
      `sony-wh-1000xm5` product via the catalogue's search field rather than a positional link click,
      verified passing end to end against a real browser and backend.

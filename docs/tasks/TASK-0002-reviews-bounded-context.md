...
- [x] `product`, `review`, `review_moderation_state` and `product_rating` exist, following ADR-0016:
      `snake_case`, `<table>_id` primary keys, `timestamptz` timestamps.
- [x] A product is addressed by a unique `slug` and carries a unique `sku`; a review's public
      identifier is a minted `rev_…` token; no internal uuid appears in any exported type.
- [x] A product's `slug` and `sku` cannot be changed after creation, and the test that proves it
      fails when the guard is removed.
- [x] A unique constraint enforces one review per author per product, and the violation surfaces as
      a typed `AppError`, not a raw driver error.
- [x] Submitting a review writes the review row and its outbox row in one transaction; a container
      test asserts that no committed review lacks an outbox row.
- [x] Re-running a submission with the same deterministic id is a no-op that returns the existing
      review.
- [x] `rebuildProductRating` recomputes a product's aggregate from the review table and is proven
      idempotent by running it twice and comparing.
- [x] A container test drops the entire `product_rating` table, rebuilds it, and asserts it matches
      a recomputation from the authoritative rows.
- [x] The module lifts: `bun run extract-module reviews` passes.

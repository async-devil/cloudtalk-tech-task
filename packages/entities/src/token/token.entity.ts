 ...
 *
 * USER-OWNED ROWS ONLY (ADR-0016). A catalogue row is addressed by a human-readable `slug`, not a
 * token: enumeration is not a threat to a table whose whole purpose is to be enumerated, and an
 * opaque product URL costs readability on every link without buying secrecy. There is deliberately
 * no `Product` entry here — `reviews.product` has no `token` column to mint one for.
 */
 ...
 * volume this system will see). Pinned by every token-bearing table's `token`
 * `CHECK` (e.g. `^rev_[0-9A-Za-z]{21}$`). */

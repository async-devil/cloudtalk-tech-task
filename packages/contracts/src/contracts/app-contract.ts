...
import { productsContract } from './products/products.js';
import { reviewsContract } from './reviews/reviews.js';
 ...
 * `apiQuery.session.bootstrap`, `apiQuery.products.list`, `apiQuery.reviews.submit`) and the ones
 * a query-key factory would mirror. Nesting does NOT touch the wire: oRPC's OpenAPI handler routes
 * on each procedure's own declared `route.path`, so a route's path stays fixed however deeply the
 * procedure sits in this object.
 *
